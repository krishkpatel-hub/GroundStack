...
2. Work from `deploy/simplified-live-demo`.
...
3. From a local checkout with private Neon/provider variables set outside the repository,
   run `make migrate-production` with `CONFIRM_PRODUCTION_MIGRATION=yes`.
4. Run `make db-smoke`.
